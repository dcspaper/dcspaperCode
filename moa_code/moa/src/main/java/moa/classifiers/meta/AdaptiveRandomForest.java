...
public class AdaptiveRandomForest extends AbstractClassifier implements MultiClassClassifier,
    ...
    public MultiChoiceOption mFeaturesModeOption = new MultiChoiceOption("mFeaturesMode", 'o', 
        ...
        new String[]{"Specified m (integer value)", "sqrt(M)+1", "M-(sqrt(M)+1)",
            "Percentage (M * (m / 100))", "Decay"},
        new String[]{"SpecifiedM", "SqrtM1", "MSqrtM1", "Percentage", "Decay"}, 1);
    ...
    protected static final int FEATURES_DECAY = 4;
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        for (int i = 0 ; i < this.ensemble.length ; i++) {
            ...
            // keeps the prediction for posterior kappa computation
            int predictedClass = Utils.maxIndex(vote.getArrayRef());
            this.ensemble[i].lastPredictions.add(predictedClass);
...
//        System.out.print("\n");
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        LinkedList<Measurement> allMeasurements = new LinkedList<>();
        if(ensemble != null) {
            for (ARFBaseLearner tree : ensemble) {
                allMeasurements.add(new Measurement("tree depth", tree.classifier.measureTreeDepth()));
            }
            for (ARFBaseLearner tree : ensemble) {
                allMeasurements.add(new Measurement("tree acc", tree.evaluator.getFractionCorrectlyClassified()));
            }
        }
        ...
        // Kappa time
//        double sumK = 0.0;
//        int qtdComparisons = 0;
//        for(int i = 0; i < ensemble.length; i++){
//            Integer[] votesA = (Integer[]) ensemble[i].lastPredictions.toArray(new Integer[0]);
//            for(int j = i + 1; j < ensemble.length; j++){
//                Integer[] votesB = (Integer[]) ensemble[j].lastPredictions.toArray(new Integer[0]);
//                double k = computeKappa(votesA, votesB);
//                sumK += k;
//                qtdComparisons++;
//                allMeasurements.add(new Measurement("kappa - (" + (i + 1) + ", " + (j + 1) + ")", k));
//            }
//        }
//
//        allMeasurements.add(new Measurement("avg kappa", sumK / qtdComparisons));

        // clears the buffers
        for (AdaptiveRandomForest.ARFBaseLearner l : ensemble){
            l.lastPredictions.clear();
        }



        for(int i = 0; i < ensemble.length; i++){
            allMeasurements.add(new Measurement("acc " + (i+1),
                    100.0 * ensemble[i].evaluator.getFractionCorrectlyClassified()));
        }

        Measurement[] arfMeasurements = (Measurement[]) allMeasurements.toArray(new Measurement[0]);
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
        ...
        switch(this.mFeaturesModeOption.getChosenIndex()) {
            ...
            case AdaptiveRandomForest.FEATURES_PERCENT:
                ...
                break;
            case AdaptiveRandomForest.FEATURES_DECAY:
                this.subspaceSize = 1;
                useDecay = true;
                break;
        }
        ...
        treeLearner.decaySubspaceOption.setValue(useDecay);
    ...
    public final class ARFBaseLearner extends AbstractMOAObject {
        ...
        public ArrayList<Integer> lastPredictions;
    ...
    public ARFBaseLearner[] getEnsemble() {
        return ensemble;
    }

    private double computeKappa(Integer[] y1, Integer[] y2){
