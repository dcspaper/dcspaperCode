...
public class HoeffdingAdaptiveTreeClassifLeaves extends HoeffdingAdaptiveTree {
	...
	return new LearningNodeHATClassifier(initialClassObservations, null, this, depth);
    }

    //@Override
    protected LearningNode newLearningNode(double[] initialClassObservations, Classifier cl, int depth) {
	return new LearningNodeHATClassifier(initialClassObservations, cl, this, depth);
    }
	...
	if (!node.observedClassDistributionIsPure()) {
		...
		if (splitDecision.splitTest == null) {
		    ...
		    SplitNode newSplit = newSplitNode(splitDecision.splitTest,
			    node.getObservedClassDistribution(), parent.depth + 1);
		    for (int i = 0; i < splitDecision.numSplits(); i++) {
			//Unique Change of HoeffdingTree
			Node newChild = newLearningNode(splitDecision.resultingClassDistributionFromSplit(i), ((LearningNodeHATClassifier) node).getClassifier(), newSplit.depth + 1);
	...
	public LearningNodeHATClassifier(double[] initialClassObservations, int depth) {
	    super(initialClassObservations, depth);
	}

	public LearningNodeHATClassifier(double[] initialClassObservations,
		Classifier cl,
		HoeffdingAdaptiveTreeClassifLeaves ht, int depth) {
	    super(initialClassObservations, depth);
