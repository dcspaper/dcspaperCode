...
public class HoeffdingTree extends AbstractClassifier implements MultiClassClassifier,
    ...
    public IntOption maxTreeDepthOption = new IntOption("maxTreeDepth", 'T', "", -1);
    ...
    public static class Node extends AbstractMOAObject {
        ...
        public Node(double[] classObservations, int depth) {
    ...
    public static class SplitNode extends Node {
        ...
        public SplitNode(InstanceConditionalTest splitTest,
                         double[] classObservations, int size, int depth) {
            super(classObservations, depth);
            this.splitTest = splitTest;
            this.children = new AutoExpandVector<Node>(size);
        }

        public SplitNode(InstanceConditionalTest splitTest,
                         double[] classObservations, int depth) {
            super(classObservations, depth);
            this.splitTest = splitTest;
    ...
    public static abstract class LearningNode extends Node {
        ...
        public LearningNode(double[] initialClassObservations, int depth) {
    ...
    public static class InactiveLearningNode extends LearningNode {
        ...
        public InactiveLearningNode(double[] initialClassObservations, int depth) {
            super(initialClassObservations, depth);
        }
    ...
    public static class ActiveLearningNode extends LearningNode {
        ...
        public ActiveLearningNode(double[] initialClassObservations, int depth) {
            super(initialClassObservations, depth);
    ...
    protected SplitNode newSplitNode(InstanceConditionalTest splitTest,
                                     double[] classObservations, int size, int depth) {
        return new SplitNode(splitTest, classObservations, size, depth);
    }

    protected SplitNode newSplitNode(InstanceConditionalTest splitTest,
                                     double[] classObservations, int depth) {
        return new SplitNode(splitTest, classObservations, depth);
    }
    ...
    protected void attemptToSplit(ActiveLearningNode node, SplitNode parent,
                                  int parentIndex) {
        if(node.depth == maxTreeDepthOption.getValue()) {
            return;
        }
            ...
            if (shouldSplit) {
                ...
                if (splitDecision.splitTest == null) {
                    ...
                    SplitNode newSplit = newSplitNode(splitDecision.splitTest,
                            node.getObservedClassDistribution(), splitDecision.numSplits(), node.depth);
                    for (int i = 0; i < splitDecision.numSplits(); i++) {
                        Node newChild = newLearningNode(splitDecision.resultingClassDistributionFromSplit(i), newSplit.depth + 1);
    ...
    protected void deactivateLearningNode(ActiveLearningNode toDeactivate,
        ...
        int parentDepth = 0;
        if(parent != null) parentDepth = parent.depth;
        Node newLeaf = new InactiveLearningNode(toDeactivate.getObservedClassDistribution(), parentDepth + 1);
        if (parent == null) {
    ...
    protected void activateLearningNode(InactiveLearningNode toActivate,
        ...
        int parentDepth = 0;
        if(parent != null) parentDepth = parent.depth;
        Node newLeaf = new InactiveLearningNode(toActivate.getObservedClassDistribution(), parentDepth + 1);
        if (parent == null) {
            this.treeRoot = newLeaf;
        } else {
            parent.setChild(parentBranch, newLeaf);
    ...
    public static class LearningNodeNB extends ActiveLearningNode {
        ...
        public LearningNodeNB(double[] initialClassObservations, int depth) {
            super(initialClassObservations, depth);
        }
    ...
    public static class LearningNodeNBAdaptive extends LearningNodeNB {
        ...
        public LearningNodeNBAdaptive(double[] initialClassObservations, int depth) {
            super(initialClassObservations, depth);
        }
    ...
    protected LearningNode newLearningNode(int depth) {
        return newLearningNode(new double[0], depth);
    }
    ...
    protected LearningNode newLearningNode(double[] initialClassObservations, int depth) {
        ...
        if (predictionOption == 0) { //MC
            ret = new ActiveLearningNode(initialClassObservations, depth);
        } else if (predictionOption == 1) { //NB
            ret = new LearningNodeNB(initialClassObservations, depth);
        } else { //NBAdaptive
            ret = new LearningNodeNBAdaptive(initialClassObservations, depth);
        }
        return ret;
    }
